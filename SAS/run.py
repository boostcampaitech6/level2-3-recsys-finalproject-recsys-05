...
def main(cfg: CFG):
    ...
    df = pd.read_csv(os.path.join(cfg['data_dir'], cfg['data_file']))
    logger.info("## csv data loaded")

    layer_col = [x for x in cfg['cate_cols'] if x != 'summonerId' and x != 'matchId' and x != 'position']
    cfg['n_layers'] = df[layer_col].max().max() + 1

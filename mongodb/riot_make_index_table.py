    ...
    for cate in cate_cols.keys():
        if cate == 'other':
            for col in cate_cols[cate]:
                ...
                unique = mongo.get_unique_values('riot_match', col, batch_size)

                await mongo.make_index_table(unique, f'riot_match_{col}_to_index', batch_size)
            ...
            for col in cate_cols[cate]:
                ...
                unique = unique.union(mongo.get_unique_values('riot_match', col, batch_size))

            await mongo.make_index_table(unique, f'riot_match_{cate}_to_index', batch_size)
